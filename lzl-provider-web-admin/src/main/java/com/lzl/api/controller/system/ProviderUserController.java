...
public class ProviderUserController extends BaseController  {
    ...
    // 前台登陆页面传来的是一个ProviderUser对象，通过此处的Controller的处理，
    // 到底层数据库中查询出该登陆用户的所有权限对应的urls，
    // 并将该urls加入到该ProviderUser对象中返回给前台，前台使用该urls进行页面的显示
    ...
    public ResponseEntity<Object> doLogin(HttpServletRequest request,@RequestBody ProviderUser user) throws Exception {
        ...
        System.out.println("scope:" + ProviderResources.S_PROVIDER);
        List<String> resourceList = providerResourcesService.getResourceByRoleId(providerUser.getProviderRolesId(), ProviderResources.S_PROVIDER);
    ...
    public ResponseEntity<Object> put(HttpServletRequest request,@RequestBody ProviderUser providerUser) throws Exception {
		System.out.println("更新providerUser时，前台传来的providerUser：" + providerUser);
		providerUserService.put(providerUser);
		return new ResponseEntity<Object>(HttpStatus.OK);
    }
	
    @RequestMapping(value = "/{userId}", method = { RequestMethod.DELETE })
    public ResponseEntity<Object> delete(HttpServletRequest request,@PathVariable("userId") Integer userId) throws Exception {
    	providerUserService.deleteProviderUserById(userId);
        return new ResponseEntity<Object>(HttpStatus.OK);
    }
