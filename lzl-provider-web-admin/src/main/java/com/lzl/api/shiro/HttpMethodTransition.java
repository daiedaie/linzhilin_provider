...
/**
 * org.apache.shiro.web.filter.authz.HttpMethodPermissionFilter 改造过来的
 * 用来转换http的method成对应的shiro里面的create,read,update,delete;
 * 2018-1-26 11:23:27
 * @since 1.0
 */
public class HttpMethodTransition {
    ...
    private static final HttpMethodTransition single = new HttpMethodTransition();

    //静态工厂方法   
    public static HttpMethodTransition getInstance() {
        return single;
    }
    ...
    private static final Logger       log               = LoggerFactory
        .getLogger(HttpMethodTransition.class);
    ...
    private static enum HttpMethodAction {
                                          ...
                                          DELETE(DELETE_ACTION), GET(READ_ACTION), HEAD(READ_ACTION), MKCOL(CREATE_ACTION), //webdav, but useful here
                                          OPTIONS(READ_ACTION), POST(CREATE_ACTION), PUT(UPDATE_ACTION), TRACE(READ_ACTION);
     ...
     *         will allow the request to continue.
     */
    protected String[] buildPermissions(HttpServletRequest request, String[] configuredPerms,
                                        String action) {
